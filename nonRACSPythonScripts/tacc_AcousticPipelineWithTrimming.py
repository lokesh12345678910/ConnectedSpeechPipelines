...
rVADPath = sys.argv[3] # "/work/07469/lpugalen/ls6/rVADfast/rVAD_fast.py"
import math
import parselmouth
...
def syllable_nuclei(fileAddress, fileName):
    silencedb = -25
    mindip = 2
    minpause = 0.3
...
def extractAndOutputAcousticFeatures(fileAddress, fileName):
  import opensmile
  ...
  GeMAPS_df = smile.process_file(fileAddress)
  syllable_nuclei_dictionary = syllable_nuclei(fileAddress, fileName)
  import pandas as pd
  allAcousticFeatures_df = GeMAPS_df
...
def extractAWSIDForAcoustic(sampleName):
  temp = sampleName.split('/')[-1]
...
def runPipeline(inputDirectory, fileName):
  ...
  for fileAddress in listOfFiles:
    ...
    sampleName=fileAddress
    fileAcousticFeats = extractAndOutputAcousticFeatures('trimmed_' + fileAddress, sampleName)
    acousticFeats.append(fileAcousticFeats)
    os.system("rm trimmed_" + fileAddress)
    os.system("rm mono_"+ fileAddress)
  import pandas as pd
  ...
  directoryAcousticFeats.insert(0,'AWS_ID', directoryAcousticFeats['file'].apply(extractAWSIDForAcoustic))
  print("Shape of Acoustic Output:", directoryAcousticFeats.shape)
  directoryAcousticFeats.to_csv(fileName + "_AcousticFeatures.csv", index=False)
